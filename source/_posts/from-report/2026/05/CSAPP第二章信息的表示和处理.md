...
当 $x+y \ge 2^w$ 时，实际结果为 $s = x+y-2^w$

对任意的 $x+y$，有 $s = (x+y) \bmod 2^w$

**溢出的结果：** 和小于两个加数

**检验溢出的方式：** 如果 $s<x$，说明溢出

**无符号数的非：** $x \mapsto 2^w - x \ (x>0)$

### **2.3.2 补码加法**

当 $x+y \ge 2^{w-1}$ 时，$s = x+y-2^w$

当 $x+y < -2^{w-1}$ 时，$s = x+y+2^w$

**正溢出的结果是负数，负溢出的结果是正数。**

**检验溢出的方式：** 当 $x,y>0$ 而 $s\le 0$ 时是正溢出；当 $x,y<0$ 而 $s\ge 0$ 时是负溢出
...
小数的二进制表示法只能表示那些能够写为 **$x \times 2^w$** 的数，**其他的数都是近似表示。** $x$ 必须可以由形如 $2^i + 2^j + \dots + 2^n$ 的多项式表示。

浮点运算的不精确性可能产生严重后果

### **2.4.2 IEEE 浮点表示**

**IEEE 浮点标准**的表示形式为：$V = (-1)^S \times M \times 2^E$，它分为三部分：

 **符号**：**S**  决定是负数还是正数

 **阶码**：**E**  的作用是对浮点数加权

 **尾数**：**M** 是一个二进制小数。规格化值时范围是 $1 \le M < 2$，非规格化值时范围是 $0 \le M < 1$
 ...
 **规格化的值：** 阶码字段即不全为 0 也不全为 1 时属于规格化值（0001~1110）
 ...
 阶码字段解释方式：$E = e - (2^{k-1}-1)$；如 $k=4$ 时，$E$ 的范围是 $-6$ 到 $7$；单精度为 $-126$ 到 $127$
...
2.  使用 int32\_t, int64\_t, uint32\_t, uint64\_t。定义在头文件 \<stdint.h\> 中
...
10.  int 大约是 10 位十进制精度（约 $-20$ 亿到 $20$ 亿）；long long 大约是 19 位；char 为 $-128$ 到 $127$；short 为 $-32768$ 到 $32767$
11.  **6位**，10 的 38 次方以内；**15位**，10 的 308 次方以内；
12.  最高位解释为负权
13.  位值不变，改变解释的方式
14.  若 $x<0$，则 $u_x = x + 2^w$；若无符号数 $u_x \ge 2^{w-1}$，则 $x = u_x - 2^w$
15.  左端填充为符号位；左端填充为 0
16.  对 2\^k 取模
...
10.  规格化值：阶码非全 0 且非全 1，指数偏移量为 $2^{k-1}-1$，尾数为 $1+f$；非规格化值：阶码全 0，$E = 1-(2^{k-1}-1)$，尾数为 $f$；特殊值：阶码全 1，尾数全 0 表示无穷，尾数非全 0 表示 NaN
